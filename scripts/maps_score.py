...
def bed_to_hail_interval(bed_file, ref='GRCh37'):
    ...
    def format_interval(x):
        chr = x.chrom
        if chr_style == '' and 'chr' in chr:
            chr = chr.replace('chr', '')
        elif chr_style == 'chr' and 'chr' not in chr:
            chr = f'chr{chr}'
        return f'{chr}:{x.start + 1}-{x.stop + 1}'
...
def count_variants(ht: hl.Table,
                   count_singletons: bool = False, count_downsamplings=(),
                   additional_grouping=(), partition_hint: int = 100,
    ...
    if count_downsamplings or force_grouping:
        ...
        output = {'variant_count': hl.agg.count_where(
            ht.freq[0].AF <= 0.001) if impose_high_af_cutoff_here else hl.agg.count()}
        for pop in count_downsamplings:
            output[f'downsampling_counts_{pop}'] = downsampling_counts_expr(ht, pop,
                                                                            impose_high_af_cutoff=impose_high_af_cutoff_here)
        if count_singletons:
...
if __name__ == '__main__':
    bed_file = str(snakemake.input["bed"])
    gnomad_path = str(snakemake.input["gnomAD"])
    mutation_path = str(snakemake.config["gnomAD"]["mutation_rate_ht"])
    out_dir = snakemake.output["maps"]

    threads = snakemake.threads
    hl.init(local=f"local[{threads}]")

    intervals = bed_to_hail_interval(bed_file)
